<?php
	/*
	Plugin Name: Zu Slider
	Plugin URI: http://www.zuthemes.com
	Description: Zu Slider is a WordPress plugin to display galleries as sliders, using WooThemes popular FlexSlider plugin.
	Version: 0.0.1
	Author: ZuThemes
	Author URI: http://www.zuthemes.com
	License: GPL2
	*/
	...
	class ZuFlexSlider {
		...
		public static function flexslider_shortcode($atts, $content = null) {
		    ...
		    return ZuFlexSlider::get_gallery($id);
		}
		...
		public static function flexslider_admin() {
		    add_meta_box( 'flexslider_meta_box',
		        'Zu Slider Shortcode - Paste it where you need it',
		        array('ZuFlexSlider', 'display_flexslider_meta_box'),
		        'flexslide', 'normal', 'high'
		    );
		}

		public static function display_flexslider_meta_box( $movie_review ) {
			echo '[flexslider id="' . get_the_ID() . '"]';
		}
	}
	...
	add_action( 'init', array('ZuFlexSlider', 'create_post_type') );

	// Register stylesheet
	add_action( 'wp_enqueue_scripts', array('ZuFlexSlider', 'add_wpflexslider_stylesheet'));

	// Register flexslider javascript
	add_action('wp_enqueue_scripts', array('ZuFlexSlider', 'add_wpflexslider_script'));

	// Register main javascript
	add_action('wp_enqueue_scripts', array('ZuFlexSlider', 'add_main_script'));

	// Register shortcode
	add_shortcode('flexslider',  array('ZuFlexSlider', 'flexslider_shortcode'));

	// Register widget to display ID when editing
	add_action( 'admin_init', array('ZuFlexSlider','flexslider_admin') );
?>
